...
from .db_models import db, User, Item, Order
...
def home():
        if current_user.is_authenticated:
                tname = current_user.name
                items = Item.query.all()
                return render_template("home.html", items=items, name=tname)
        else:
                tname = 'customer'
                items = Item.query.all()
                return render_template("home.html", items=items, name=tname)
...
def create_checkout_session():
        ...
        phoneno = current_user.phone
        #user = load_user(session)
        #current_name = current_user.users.name
        fulfill_order(session, user, phoneno)
        return render_template('success.html')
